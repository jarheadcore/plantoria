...
<template>
    <section class="w-full shrink-0 snap-start snap-always overflow-y-auto p-4">
        ...
        <div class="flex items-center gap-3 mb-5">
            ...
            <div>
                <h2 class="text-xl font-bold text-green-800">Garten-Reise</h2>
                <p class="text-sm text-green-600">Was wächst wann?</p>
            </div>
        </div>
        ...
        <div class="space-y-2">
            <div v-for="(group, gi) in store.eventsBySeason" :key="group.id">
                <!-- Season banner -->
                <div class="flex items-center gap-3 px-4 py-3 rounded-2xl mb-3" :class="group.bgColor">
                    <span class="text-3xl">{{ group.emoji }}</span>
                    <span class="text-lg font-bold" :class="group.textColor">
                        {{ group.name }}
                    </span>
                </div>

                <!-- Path with events -->
                <div class="ml-6 pl-6 space-y-4 border-l-4" :class="group.borderColor">
                    <div v-for="(event, ei) in group.events" :key="ei" class="relative">
                        ...
                        <!-- Event card -->
                        <div class="flex items-center gap-4 p-4 bg-white rounded-2xl shadow-sm" :class="event.done ? 'opacity-60' : ''">
                            <span class="text-4xl shrink-0">{{ event.cropEmoji }}</span>

                            <div class="flex-1 min-w-0">
                                <p class="font-bold text-lg" :class="event.done ? 'line-through text-gray-400' : ''">
                                    {{ event.task }}
                                </p>
                                <span
                ...
                <div v-if="gi < store.eventsBySeason.length - 1" class="ml-6 h-6 border-l-4 border-dashed border-gray-200" />
            </div>
        </div>

        <!-- Encouraging footer -->
        <div class="mt-5 p-5 bg-green-100 rounded-2xl text-center">
            <p class="text-base text-green-800 font-medium">🌻 Am Ende wartet eine tolle Ernte auf euch!</p>
        </div>
    </section>
</template>

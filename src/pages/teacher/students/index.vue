...
const selectedProjectGroups = computed(() => projectsStore.getGroupsByProjectId(selectedProjectId.value))
...
const allStudentOptions = computed(() => studentsStore.students.map((s) => ({ label: s.name, value: s.id })))
    ...
    <div>
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex gap-2">
                <UButton color="primary" size="sm" @click="openCreateGroup"> <Plus :size="16" class="mr-1" /> Neue Gruppe </UButton>
            </div>
        </div>
            ...
            <UCard v-for="group in selectedProjectGroups" :key="group.id">
                <div class="flex items-start justify-between mb-2">
                    ...
                    <div class="flex gap-1">
                        <UButton v-if="group.name !== 'Nicht zugewiesen'" size="xs" variant="ghost" @click="openEditGroup(group)">
                            <Pencil :size="14" />
                        </UButton>
                        <UButton
                ...
                <div class="flex flex-wrap gap-1 mb-2">
                    <UBadge v-for="name in getStudentNames(group.studentIds)" :key="name" color="neutral" variant="subtle" size="xs">
                        {{ name }}
                    </UBadge>
                    <span v-if="group.studentIds.length === 0" class="text-xs text-gray-400 italic"> Keine Schüler zugewiesen </span>
                </div>

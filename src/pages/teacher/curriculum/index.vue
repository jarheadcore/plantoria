...
<template>
  <div>
      ...
      <UCard v-for="cur in curriculumStore.curriculumData" :key="cur.id">
        ...
        <div class="space-y-3">
          <div
            ...
            :key="topic.id"
            :class="['rounded-lg border p-3', topic.treated ? 'border-green-200 bg-green-50/50 dark:border-green-900 dark:bg-green-950/20' : 'border-gray-100 dark:border-gray-800']"
          >
            <div class="flex items-start gap-3">
              <input
                type="checkbox"
                :checked="topic.treated"
                disabled
      ...
      <UCard>
        ...
        <div class="space-y-1">
          <p v-for="cur in curriculumStore.curriculumData" :key="cur.id">
            <span v-for="topic in cur.topics.filter(t => t.treated && t.treatedByProject === 'Gemüsefläche HE24a')" :key="topic.id" class="block text-sm">
    ...
    <div v-if="activeTab === 'chronological'" class="space-y-2">
      <div
        v-for="cur in curriculumStore.curriculumData"
        :key="cur.id"
      >
        <div
          v-for="topic in cur.topics.filter(t => t.treated).sort((a, b) => (b.treatedDate || '').localeCompare(a.treatedDate || ''))"
          :key="topic.id"

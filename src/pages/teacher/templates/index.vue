...
const currentTemplates = computed(() => {
    ...
    switch (activeTab.value) {
        case 'own':
            list = templatesStore.ownTemplates
            break
        case 'shared':
            list = templatesStore.sharedTemplates
            break
        case 'platform':
            list = templatesStore.platformTemplates
            break
    }
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div class="flex-1">
                        ...
                        <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 mb-3">
                            <span
                                >Kategorie: <strong>{{ tmpl.category }}</strong></span
                            >
                            <span
                                >Stufe: <strong>{{ tmpl.gradeRange }}</strong></span
                            >
                            <span>
                                Schwierigkeit:
                            ...
                            <span
                                >Fachbereiche: <strong>{{ tmpl.topics.length }}</strong></span
                            >
                            <span
                                >Aufgaben: <strong>{{ getTaskTemplateCount(tmpl.id) }}</strong> ({{ getTaskTemplatesWithLp21(tmpl.id) }} mit
                                LP21)</span
                            >
                            <span v-if="tmpl.usedBySchools"
                                >Genutzt von: <strong>{{ tmpl.usedBySchools }} Schulen</strong></span
                            >
                            <span v-if="tmpl.createdBy !== 'Plantoria'"
                                >Erstellt von: <strong>{{ tmpl.createdBy }}</strong></span
                            >
                        </div>
                        <div class="max-w-xs">
                    ...
                    <div class="flex flex-wrap gap-2 shrink-0">
                        <UButton v-if="tmpl.isOwn" size="sm" variant="soft" :to="`/teacher/templates/${tmpl.id}`"> Bearbeiten </UButton>
                        <UButton v-if="tmpl.isOwn" size="sm" variant="soft" color="primary" disabled> Teilen </UButton>
                        <UButton v-if="!tmpl.isOwn" size="sm" variant="soft" color="primary" disabled> Importieren </UButton>
                        <UButton size="sm" variant="ghost" color="neutral" disabled> Vorschau </UButton>
                        <UButton size="sm" color="primary" to="/teacher/projects/new"> Projekt erstellen </UButton>
                    </div>
                </div>
            </UCard>
        </div>

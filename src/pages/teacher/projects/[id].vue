...
const monthTopics = computed(() => {
    return months.map((name, idx) => {
        ...
        const active = topics.value.filter((t) => monthNum >= t.startMonth && monthNum <= t.endMonth)
...
<template>
    <div v-if="project">
        <!-- Breadcrumb -->
        <div class="mb-4">
            <NuxtLink to="/teacher/projects" class="text-sm text-gray-500 hover:text-gray-700"> ← Projekte </NuxtLink>
        </div>
        ...
        <div class="mb-6">
            ...
            <div class="flex flex-wrap items-center gap-4 mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <USelect :model-value="project.status" :items="statusOptions" size="sm" @update:model-value="onStatusChange" />
                </div>
                <div class="flex items-center gap-2">
                    <span class="text-sm text-gray-500">Phase:</span>
                    <USelect :model-value="project.currentPhase" :items="phaseOptions" size="sm" @update:model-value="onPhaseChange" />
                </div>
                ...
                <div class="min-w-[800px]">
                    ...
                    <div class="flex mb-1">
                        ...
                        <div
                            v-for="(m, i) in months"
                            :key="i"
                            :class="['flex-1 text-center text-xs', i === currentMonth ? 'font-bold text-green-600' : 'text-gray-400']"
                        >
                            {{ m }}
                        </div>
                    </div>
                        ...
                        >
                            ...
                            <div class="flex-1 relative h-6">
                                <div
                                    :class="[
                                        topic.color,
                                        'absolute top-0.5 bottom-0.5 rounded opacity-80 group-hover:opacity-100 transition-opacity',
                                    ]"
                                    :style="{
                                ...
                                >
                                    <span
                                        class="absolute inset-0 flex items-center justify-center text-[10px] font-medium text-white drop-shadow truncate px-1"
                                    >
                ...
                <div v-for="phase in phases" :key="phase.key">
                    ...
                    >
                        ...
                        <div class="divide-y divide-gray-100 dark:divide-gray-800">
                            <div v-for="topic in topicsByPhase[phase.key]" :key="topic.id" :id="`topic-${topic.id}`">
                                ...
                                >
                                    ...
                                    <div class="flex items-center gap-3">
                                        <span class="text-xs text-gray-400"
                                            >{{ monthLabel(topic.startMonth) }}–{{ monthLabel(topic.endMonth) }}</span
                                        >
                                    ...
                                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <div class="flex items-center justify-between mb-2">
                                                ...
                                                <UButton v-if="topic.tutorialVideoUrl" size="xs" variant="soft" color="primary" disabled>
                                                    ▶ Video
                                                </UButton>
                                            </div>
                    ...
                    <div v-if="selectedVegetable" class="p-6">
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div v-if="selectedVegetable.sowingIndoor" class="text-sm">
                                ...
                                <span class="ml-1 font-medium"
                                    >{{ monthLabel(selectedVegetable.sowingIndoor.startMonth) }}–{{
                                        monthLabel(selectedVegetable.sowingIndoor.endMonth)
                                    }}</span
                                >
                            </div>
                            <div v-if="selectedVegetable.sowingOutdoor" class="text-sm">
                                ...
                                <span class="ml-1 font-medium"
                                    >{{ monthLabel(selectedVegetable.sowingOutdoor.startMonth) }}–{{
                                        monthLabel(selectedVegetable.sowingOutdoor.endMonth)
                                    }}</span
                                >
                            </div>
                            <div class="text-sm">
                                ...
                                <span class="ml-1 font-medium"
                                    >{{ monthLabel(selectedVegetable.harvestPeriod.startMonth) }}–{{
                                        monthLabel(selectedVegetable.harvestPeriod.endMonth)
                                    }}</span
                                >
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-4 text-sm">
                            <div>
                                ...
                                <span class="ml-1"
                                    >{{ selectedVegetable.spacing.inRow }}cm / {{ selectedVegetable.spacing.betweenRows }}cm</span
                                >
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <span class="ml-1"
                                    >{{ selectedVegetable.germination.tempMin }}–{{ selectedVegetable.germination.tempMax }}°C,
                                    {{ selectedVegetable.germination.days }} Tage</span
                                >
                            </div>
                            <div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="flex flex-wrap gap-1">
                                <UBadge v-for="c in selectedVegetable.companions" :key="c" color="green" variant="subtle" size="xs">
                                    {{ c }}
                                </UBadge>
                            </div>
                        </div>
                        <div v-if="selectedVegetable.antagonists.length > 0" class="mb-3">
                            ...
                            <div class="flex flex-wrap gap-1">
                                <UBadge v-for="a in selectedVegetable.antagonists" :key="a" color="neutral" variant="subtle" size="xs">
                                    {{ a }}
                                </UBadge>
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <UCard v-for="group in groups" :key="group.id">
                    <div class="flex items-start justify-between mb-2">
                        <h3 class="font-semibold">{{ group.name === 'Nicht zugewiesen' ? '⚠️ ' : '🌻 ' }}{{ group.name }}</h3>
                        <UButton v-if="group.name !== 'Nicht zugewiesen'" size="xs" variant="ghost" disabled> Bearbeiten </UButton>
                        <UButton v-else size="xs" variant="soft" color="primary" disabled> Zuweisen </UButton>
                    </div>
        ...
        <div v-if="activeTab === 'preproject' && preProject">
            <div class="mb-4 flex items-center gap-4">
                <UBadge :color="preProject.status === 'Abgeschlossen' ? 'green' : 'neutral'" variant="subtle">
                    {{ preProject.status }}
                </UBadge>
                <div class="flex-1">
                ...
                <div v-for="(items, category) in preProjectByCategory" :key="category">
                    ...
                    <div class="space-y-2">
                        ...
                        >
                            ...
                            <UButton v-if="item.orderUrl && !item.completed" size="xs" variant="soft" disabled> Bestellen → </UButton>
                        </div>
                    </div>
                </div>
            </div>

            <UButton color="primary" class="mt-6" :disabled="preProjectProgress < 100"> Vorprojekt abschliessen </UButton>
        </div>
    </div>

    <div v-else class="py-16 text-center text-gray-500">Projekt nicht gefunden.</div>
</template>

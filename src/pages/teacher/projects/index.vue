...
<template>
    <div>
        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <UInput v-model="searchQuery" placeholder="Suche..." :ui="{ leading: 'pointer-events-none' }" class="w-full sm:w-64">
                <template #leading>
        ...
        <!-- Project cards -->
        <div class="space-y-4">
            <NuxtLink v-for="project in filteredProjects" :key="project.id" :to="`/teacher/projects/${project.id}`" class="block">
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap gap-x-6 gap-y-1 text-sm text-gray-500">
                                <span
                                    >Phase: <strong>{{ project.currentPhase }}</strong></span
                                >
                                <span v-if="project.area"
                                    >Fläche: <strong>{{ project.area }}m²</strong></span
                                >
                                <span
                                    >Aufgaben:
                                    <strong
                                        >{{ projectsStore.getProjectStats(project.id).tasksDone }}/{{
                                            projectsStore.getProjectStats(project.id).taskCount
                                        }}</strong
                                    ></span
                                >
                                <span v-if="projectsStore.getProjectStats(project.id).groupCount"
                                    >Gruppen: <strong>{{ projectsStore.getProjectStats(project.id).groupCount }}</strong></span
                                >
                            </div>
                        </div>
                    </div>

...
const topicOptions = computed(() => {
    ...
    return [{ label: 'Alle Fachbereiche', value: 'all' }, ...Array.from(unique.entries()).map(([name, id]) => ({ label: name, value: id }))]
})
...
const createTopicOptions = computed(() => {
    ...
    return [{ label: 'Kein Fachbereich', value: '' }, ...topics.map((t) => ({ label: `${t.icon} ${t.name}`, value: t.id }))]
})
...
<template>
    <div>
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <UButton color="primary" @click="openCreate"> <Plus :size="16" class="mr-1" /> Neue Aufgabe </UButton>
        </div>
        ...
        <p class="mb-4 text-sm text-gray-500">{{ filteredTasks.length }} Aufgaben gefunden</p>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <UBadge v-if="task.isHolidayTask" color="neutral" variant="subtle" size="xs"> Ferienaufgabe </UBadge>
                            <UBadge :color="task.status === 'Erledigt' ? 'primary' : 'neutral'" variant="subtle" size="xs">
                                {{ task.status }}
                            </UBadge>
                        </div>
                    ...
                    <div class="flex items-center gap-1 shrink-0">
                        ...
                        <UButton size="xs" variant="ghost" color="neutral" @click="tasksStore.deleteTask(task.id)"> &times; </UButton>
                    </div>
                </div>
            </UCard>
        </div>

        <p v-if="filteredTasks.length === 0" class="py-12 text-center text-gray-400">Keine Aufgaben gefunden.</p>

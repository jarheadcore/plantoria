...
import type { Material, RecentDownload, SubjectArea, Difficulty, ProjectPhase, FileFormat } from '@/types'
import { mockMaterials, mockRecentDownloads } from '@/data/mock/materials'

export const useMaterialsStore = defineStore('materials', () => {
  const materials = ref<Material[]>(mockMaterials)
  const recentDownloads = ref<RecentDownload[]>(mockRecentDownloads)

  const filterSubject = ref<SubjectArea | 'all'>('all')
  const filterGrade = ref('all')
  const filterPhase = ref<ProjectPhase | 'all'>('all')
  const filterFormat = ref<FileFormat | 'all'>('all')
  ...
  const filteredMaterials = computed(() => {
    return materials.value.filter((m) => {
      ...
      if (filterFormat.value && filterFormat.value !== 'all' && !m.formats.includes(filterFormat.value as FileFormat)) return false
  ...
  function recordDownload(materialId: string, format: FileFormat) {
